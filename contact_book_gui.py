...
# the functions
def submit_record(): 
    global name_str, email_str, phone_str, address_entry
    global cursor
    name, email, phone, address = name_str.get(), email_str.get(), phone_str.get(), address_entry.get(1.0, END)
...
def delete_all_records():
    ...
    messagebox.showinfo("All records deleted", "All records deleted")
    listbox.delete(0, END)
    list_contacts()
...
search_entry = Entry(center_frame, width=18, font=("Verdana", 12), textvariable=search_str).place(relx=0.06, rely=0.04)
